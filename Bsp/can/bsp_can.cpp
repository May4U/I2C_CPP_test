...
Bsp_CAN_n::Bsp_CAN_c bsp_CAN_instance;

////debug
uint32_t rxid_1, rxid_2, txid_1, txid_2;

/***函数声明***/

static void CANAddFilter(Bsp_CAN_n::CANInstance_t *_instance);

/**************/


/**
 * @brief 在创建一个 Bsp_CAN_c 类的时候会自动调用此函数,启动CAN服务
 *
 * @note 此函数会启动CAN1和CAN2,开启CAN1和CAN2的FIFO0 & FIFO1溢出通知
 *
 */
Bsp_CAN_n::Bsp_CAN_c::Bsp_CAN_c(/* args */)
{  
}
    
Bsp_CAN_n::Bsp_CAN_c::~Bsp_CAN_c()
{
}
...
Bsp_CAN_n::CANInstance_t *Bsp_CAN_n::CANRegister(Bsp_CAN_n::CAN_Init_Config_t *config)
{

    if(bsp_CAN_instance.idx == 0)
    {
        HAL_CAN_Start(&hcan1);
    ...
    for (size_t i = 0; i < bsp_CAN_instance.idx; i++)
    { // 重复注册 | id重复
        if (bsp_CAN_instance.can_instance[i]->rx_id == config->rx_id && bsp_CAN_instance.can_instance[i]->can_handle == config->can_handle)
        {
            rxid_1 = bsp_CAN_instance.can_instance[i]->rx_id;
            rxid_2 =  config->rx_id; 
            while (1)
            // 会卡在这里说明 某个模块的 tx或者rx id 有重复, 仔细查验
            ;
        }
    }
    
    Bsp_CAN_n::CANInstance_t *instance = (Bsp_CAN_n::CANInstance_t *)malloc(sizeof(Bsp_CAN_n::CANInstance_t)); // 分配空间
    memset(instance, 0, sizeof(Bsp_CAN_n::CANInstance_t));                           // 分配的空间所储存的数据未必是0,所以要先清空
    // 进行发送报文的配置
    ...
    bsp_CAN_instance.can_instance[bsp_CAN_instance.idx++] = instance; // 将实例保存到can_instance中
...
uint8_t Bsp_CAN_n::CANTransmit(Bsp_CAN_n::CANInstance_t *_instance, float timeout)
{
    static uint32_t busy_count;
    // static volatile float wait_time __attribute__((unused)); // for cancel warning
    float dwt_start =  Bsp_DWT_n::DWT_GetTimeline_ms();
    while (HAL_CAN_GetTxMailboxesFreeLevel(_instance->can_handle) == 0) // 等待邮箱空闲
    {
        if (Bsp_DWT_n::DWT_GetTimeline_ms() - dwt_start > timeout) // 超时
        {
    ...
    _instance->tx_wait_time = Bsp_DWT_n::DWT_GetTimeline_ms() - dwt_start;
 ...
 * @param length    设定长度
 */
void Bsp_CAN_n::CANSetDLC(Bsp_CAN_n::CANInstance_t *_instance, uint8_t length)
{
...
static void CANFIFOxCallback(CAN_HandleTypeDef *_hcan, uint32_t fifox)
{
    ...
    while (HAL_CAN_GetRxFifoFillLevel(_hcan, fifox)) // FIFO不为空,有可能在其他中断时有多帧数据进入
    {
        ...
        for (size_t i = 0; i < bsp_CAN_instance.idx; ++i)
        { // 两者相等说明这是要找的实例
            if (_hcan == bsp_CAN_instance.can_instance[i]->can_handle && rxconf.StdId == bsp_CAN_instance.can_instance[i]->rx_id)
            {
                if (bsp_CAN_instance.can_instance[i]->can_module_callback != NULL) // 回调函数不为空就调用
                {
                    bsp_CAN_instance.can_instance[i]->rx_len = rxconf.DLC;                      // 保存接收到的数据长度
                    memcpy(bsp_CAN_instance.can_instance[i]->rx_buff, can_rx_buff, rxconf.DLC); // 消息拷贝到对应实例
                    bsp_CAN_instance.can_instance[i]->can_module_callback(bsp_CAN_instance.can_instance[i]->rx_buff, rxconf.StdId);     // 触发回调进行数据解析和处理
                }
                return;
            }
        }
